...
class BERTopicClassifier:
    ...
    def classify_bertopic(self, companies_list_of_dicts:list):
        ...
        all_embeddings = [company.get('description_embeddings') for company in companies_list_of_dicts if company.get('description_embeddings') is not None]
        ...
        topic_model = BERTopic(min_topic_size=10, calculate_probabilities=True, verbose=True, nr_topics="auto")
        ...
        cleaned_topic_names = [clean_topic_name(clean_topic_name(name)) for name in topic_names]

        for i, company in enumerate(companies_list_of_dicts):
            topic_id = topics[i]
            company['derived_topic'] = cleaned_topic_names[topic_id]
            company['derived_topic_probability'] = float(probs[i][topic_id])  # Ensure it's a float
