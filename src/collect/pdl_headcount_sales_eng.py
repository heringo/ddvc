...
def insert_batch_supabase_headcount_sales_eng(company_details: typing.List[dict]):
    if len(company_details) > 0:
        print(
            f"Upserting {len(company_details)} companies headcount details (sales/eng)"
        )
        upsert_batch = sqlalchemy.dialects.postgresql.insert(
            db.schema.classes.pdl_headcount_sales_eng
...
def main():
    ...
    for i in range(0, len(company_names), MAX_NUM_RECORDS):
        ...
        for j in range(len(batch_details)):
            ...
            if info is None:
                print(
                    f"Missing 'employee_count_by_role' for company '{batch_names[j]}' (ID: {batch_ids[j]}). Skipping."
                )
