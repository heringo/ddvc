...
def insert_batch_supabase_details(company_details: typing.List[dict]):
    ...
    if len(company_details) > 0:
        ...
        upsert_batch = sqlalchemy.dialects.postgresql.insert(db.schema.classes.peopledatabase_enriched).values(company_details)
    ...
    for i in range(0, len(company_names), MAX_NUM_RECORDS):
        ...
        for j in range(len(batch_names)):
            ...
            company_details.append({
                ...
                "linkedin_follower_count": info.get('linkedin_follower_count', None),
                "employee_count_by_role": info.get('employee_count_by_role', None),
                "average_employee_tenure": info.get('average_employee_tenure', None),
